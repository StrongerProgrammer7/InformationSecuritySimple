...
/*******************/
/*
myPoint calculatePointSummPQ(myPoint p1, myPoint p2)
{
	myPoint p3;
	long long dy = 0, dx = 0, lamda = 0;
	if (p1.x != p2.x)
	{
		dy = p2.y - p1.y;
		dx = p2.x - p1.x;
		...
		if (dy < 0)
			dy += p;
		lamda = (dy * modInverse(dx, p)) % p;
		if (lamda < 0)
			lamda += p;
		p3.x = (lamda*lamda - p1.x - p2.x) % p;
		p3.y = (lamda * (p1.x - p3.x) - p1.y) % p;
		if (p3.x < 0)
			p3.x += p;
		if (p3.y < 0)
			p3.y += p;
...
myPoint calculatePointDoubling(myPoint p1)
{
	long long dy = 0, dx = 0, lamda = 0;
	myPoint p2;
	...
	lamda = (dx * modInverse(dy, p)) % p;
	p2.x = (lamda * lamda - 2 * p1.x) % p;
	p2.x = (lamda * (p1.x - p2.x) - p1.y) % p;
...
myPoint calculatePointMulty(long long num, myPoint p1)
{
	myPoint temp = p1;
	...
	while (num != 0)
	{
		if ((num % 2) != 0)
		{
			...
			{
				temp = calculatePointSummPQ(temp, p1);
			}
			num = num - 1;
		}
		num = num / 2;
